"""
GRPO-based multi-agent PPO with RNN for JAXMARL-HFT.

Implements actor-only IPPO using Group Relative Policy Optimization (GRPO).
Supports multiple agent types (e.g. MarketMaking, Execution) with separate
networks, recurrent state via ScannedLSTM, and pmap-based multi-device
parallelism.  Episode-level group advantage normalisation replaces the
GAE + critic used in standard PPO.
